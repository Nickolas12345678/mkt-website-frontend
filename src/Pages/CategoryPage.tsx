...
//             if (categoryData.id) {
//                 // Оновлення категорії через PUT
//                 const response = await axios.put(`http://localhost:8080/categories/${categoryData.id}`, categoryData, {
//                     headers: { Authorization: `Bearer ${token}` },
//                 });
//                 setCategories(categories.map(category => category.id === categoryData.id ? response.data : category));
//                 alert("Категорію успішно оновлено");
//             } else {
//                 // Створення нової категорії через POST
//                 const response = await axios.post("http://localhost:8080/categories", categoryData, {
//                     headers: { Authorization: `Bearer ${token}` },
//                 });
...
//                                         <button onClick={() => { setEditingCategory(category); setShowModal(true); }} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition">Змінити</button>
//                                         <button onClick={() => deleteCategory(category.id, category.name)} className="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition">Видалити</button>
//                                     </td> */}
//                                     <td className="p-3 text-center">
...
//                     </table>
//                 </div>
//                 <button onClick={() => { setEditingCategory(null); setShowModal(true); }} className="bg-green-600 text-white px-4 py-3 rounded-lg hover:bg-green-700 transition w-full mt-4 text-lg">Додати категорію</button>
//             </div>

//             {showModal && (
//                 <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
//                     <div className="bg-white p-6 rounded-lg shadow-lg w-96">
//                         <h2 className="text-xl font-bold mb-4 text-center text-black">{editingCategory ? "Змінити категорію" : "Додати категорію"}</h2>
//                         <input type="text" placeholder="Назва категорії" value={editingCategory ? editingCategory.name : newCategory.name} onChange={(e) => editingCategory ? setEditingCategory({ ...editingCategory, name: e.target.value }) : setNewCategory({ ...newCategory, name: e.target.value })} className="w-full p-2 border rounded mb-2 bg-gray-100 text-black" />
//                         <button onClick={saveCategory} className="bg-blue-600 text-white px-4 py-2 rounded w-full mb-2 hover:bg-blue-700 transition">{editingCategory ? "Зберегти зміни" : "Зберегти"}</button>
//                         <button onClick={() => setShowModal(false)} className="bg-gray-500 text-white px-4 py-2 rounded w-full hover:bg-gray-600 transition">Скасувати</button>
//                     </div>
//                 </div>
//             )}
//         </div>
